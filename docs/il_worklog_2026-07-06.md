...
### 2-14. 대형맵 성능 진단 & 개선 — goal_dir OOD (2026-07-07)
"32×32에서 왜 안 되나"를 진단: 모델은 5×5 FOV만 보고 **전역 정보는 `goal_dir` 하나**인데,
학습 맵이 ≤15×15라 goal_dir 성분이 **최대 14(92%는 ≤7)**에 머물렀다. 32×32에선 성분이 최대 31
(정규화 z≈9.7)까지 필요 → **학습서 못 본 OOD 영역**. 그래서 에이전트 2명(혼잡 아님)인데도 대형맵에서 실패.

- **확인 실험(무재학습)**: 평가 때 goal_dir를 학습 범위로 캡(방향 유지) → random-32-32-10 n4 0.33→0.53.
  원인이 goal_dir OOD임을 입증(단 부분적, 밀집맵은 다른 요인).
- **근본 해결(데이터)**: **큰 맵(16~32)+소수 에이전트**(goal_dir 범위) + **중간맵+다수 에이전트**(FOV 혼잡)
  212 시나리오 CBS 라벨링(전부 성공, 14,034샘플) → 기존 combined와 합쳐 **`real_v03_big.npz` 27,730샘플**
  (goal_dir 성분 최대 14→**31**로 확장). CNN 55ep 재학습 → `cnn_big.pt`.

  | map:agents | OLD(cnn_diverse) | NEW(cnn_big) | Δ |
  |---|---|---|---|
  | random-32-32-10:2 | 0.67 | 0.87 | **+0.20** |
  | random-32-32-10:4 | 0.33 | **0.67** | **+0.33** |
  | random-32-32-20:2 | 0.27 | 0.40 | +0.13 |
  | random-32-32-20:4 | 0.07 | 0.27 | **+0.20** |
  | empty-8-8 (2/4/8) | 0.93/0.53/0.27 | 1.00/0.60/0.33 | +0.07 |
  | random-32-32-*:8 | 0.20/0.00 | 0.20/0.07 | ~0 |

- **결론**: 학습 맵 크기를 키워 **goal_dir 분포 공백을 메우자 대형맵 성공률이 최대 2배**(random-32-32-10 n4).
  소형맵도 dense 데이터 덕에 소폭↑. 단 **고혼잡(n8)은 정체** — 이는 goal_dir가 아니라 교착(§2-13) 문제라
  별개 접근(더 많은 dense 데이터 / DAgger / 하이브리드)이 필요.
...
- [x] **대형맵 성능 진단·개선** — goal_dir OOD 규명, 큰 맵 학습으로 32×32 성공률 최대 2배(§2-14)
- [ ] 고혼잡(n8+) 개선 — dense 데이터 추가 / DAgger를 대형·다수 분포로 확장
