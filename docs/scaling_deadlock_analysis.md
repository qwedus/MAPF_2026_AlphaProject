...
**"시간을 늘리면 maze를 풀까?" — 아니오.** IL 롤아웃 스텝 160→600은 결과가 **완전 동일**(정책이 초반에
교착에 갇혀 그 상태로 고정 — 무한 스텝도 무의미). CBS는 타임아웃 8s→60s로 maze n4 성공률 0.00→0.40이나
인스턴스당 ~2분으로 폭발하고 n8은 여전히 불가. → maze는 **시간이 아니라 표현(전역 경로)의 문제**.

> **후속**: 이 진단을 실제로 검증했다 — `goal_dir`을 직선 벡터에서 BFS flow 힌트로 바꾸자 maze·4 성공률
> 0.00→0.67, room·4 0.00→0.60. 상세는 `docs/navhint_prototype.md`.
