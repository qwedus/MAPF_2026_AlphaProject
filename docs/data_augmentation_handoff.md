...
IL 정책의 **학습 맵**이 어떻게 만들어지고 단계적으로 증강됐는지 정리한 문서.
(평가는 별도로 MovingAI 표준 벤치를 쓴다 — 아래는 **학습용** 맵 이야기.)

모든 맵은 `map_generator.generate_map(map_type, size, num_agents, seed)`로 **절차 생성**하고,
각 시나리오를 **CBS로 풀어 정답(전문가) 라벨**을 만든 뒤 (상태, 행동) 쌍으로 병합한다.
